	...
	def add_stream(self,stream):
		...
		for i,word in enumerate(stream['words']):
			if word in self.words:
				self.word_frequencies[self.words.index(word)] = self.word_frequencies[self.words.index(word)] + stream['word_frequencies'][i]
			else:
				self.words.append(word)
				self.word_frequencies.append(stream['word_frequencies'][i])
		return 